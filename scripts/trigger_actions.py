...
def read_words() -> list[Trigger]:
    ...
    if not os.path.exists(trigger_messages_path):
        raise FileNotFoundError("Файла с ответами триггера по пути " + trigger_messages_path + " не существует!")

    try:
        triggers = json.loads(open(trigger_messages_path, 'r', encoding='UTF-8').read())
    except ValueError:
        raise ValueError(f"В файле {trigger_messages_path} присутствует ошибка!\nИсправьте и повторите попытку!")

    if type(triggers) is not dict:
        raise ValueError(f'Неправильный формат данных в файле {trigger_messages_path}!\nНеобходимый формат:'
